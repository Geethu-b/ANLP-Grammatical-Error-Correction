...
def batch_multi_pre_rec_f1(candidates, sources, gold_edits, max_unchanged_words=2, beta=0.5, ignore_whitespace_casing= False, verbose=False, very_verbose=False):
    ...
    for candidate, source, golds_set in zip(candidates, sources, gold_edits):
        ...
        for annotator, gold in golds_set.items():
            ...
            dic_gold={}
            dic_corr={}
            for a,b,c,d,e in gold:
                if(e in dic_gold):
                    dic_gold[e]+=1
                else:
                    dic_gold[e]=1
                #for summary
                if(e in dic_gold_summary):
                    dic_gold_summary[e]+=1
                else:
                    dic_gold_summary[e]=1

            for a,b,c,d,e in correct:
                if(e in dic_corr):
                    dic_corr[e]+=1
                else:
                    dic_corr[e]=1
                #for summary
                if(e in dic_corr_summary):
                    dic_corr_summary[e]+=1
                else:
                    dic_corr_summary[e]=1
            ...
            if verbose:
                ...
                print("CORRECT EDITS :", correct)
                print("GOLD SUMMARY  :", dic_gold)
                print("COR. SUMMARY  :", dic_corr)
    ...
    if verbose:
        ...
        print("R =", r)
        print("F_%.1f =" % beta, f1)
    return (p, r, f1,dic_gold_summary,dic_corr_summary)
